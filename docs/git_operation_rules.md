...
**エージェント・CI・その他自動化により `origin/main` へ push すること**を、恒常運用とはしない。このリポジトリの **ソースツリー全体（`docs/` を含む）** についても、自動化による変更は **作業ブランチ → Pull Request → マージ** とする。

すでに `main` に直接載せてしまったコミットを Pull Request と対応させる必要があるときは、適切な起点から cherry-pick 等で作業ブランチを構成し、Pull Request で統合する。統合後のツリーが意図どおりであることを `git diff` 等で確認する。
